...
def main(config):
    #res_models=torchvision.models.resnet18()
    #summary(res_models,(3,80,80))
    svname = args.name
    if svname is None:
    ...
    if args.tag is not None:
        svname += '_' + args.tag
    save_path = os.path.join('./save-b', svname)
    ...
    if config.get('load'):
        ...
        if config.get('load_encoder'):
            #model = models.load(torch.load(config['load_encoder']))
    ...
    utils.log('num params: {}'.format(utils.compute_n_params(model.encoder)))
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        utils.log('epoch {}, train {:.4f}|{:.4f} '
                'tval {:.4f}|{:.4f}, {} {}/{} {} (@{})'.format(
                epoch, aves['tl'], aves['ta'],
                aves['tvl'], aves['tva'], t_epoch, t_used, t_estimate, optimizer.state_dict()['param_groups'][0]['lr'], _sig))
