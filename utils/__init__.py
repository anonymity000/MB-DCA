...
def make_optimizer(params, name, lr, weight_decay=None, milestones=None):
    if weight_decay is None:
        weight_decay = 0.
    if name == 'sgd':
        _milestones = [100,1000]
        #print(_milestones)
        optimizer = SGD(params, lr, momentum=0.9, weight_decay=weight_decay)
        #lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size = 100, gamma = 0.01, last_epoch=-1)
        #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [50, 100, 300], gamma = 0.1, last_epoch=-1)
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones = _milestones, gamma = 0.1, last_epoch=-1)
        #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [100, 200, 300, 500], gamma = 0.1, last_epoch=-1)
        #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [50, 100], gamma = 0.1, last_epoch=-1)
    elif name == 'adam':
