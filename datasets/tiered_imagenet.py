...
class TieredImageNet(Dataset):
    ...
    def __init__(self, root_path, split='train', mini=False, **kwargs):
        split_tag = split
        image_path = os.path.join(root_path, '{}_images.pkl'.format(split_tag))
        #data = np.load(load_path, allow_pickle=True)['images']
        with open(image_path, 'rb') as f:
            data = pickle.load(f)         
        #data = np.load(load_path, allow_pickle=True)['images']
        #data = data[:, :, :, ::-1]
        label_path = os.path.join(root_path, '{}_labels.pkl'.format(split_tag))
        with open(label_path, 'rb') as f:
            label = pickle.load(f)['label_specific']
    ...
    def __getitem__(self, idx):
        image = Image.open(io.BytesIO(self.data[idx]))
